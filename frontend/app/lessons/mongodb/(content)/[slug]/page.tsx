...
import { mongodbLessons } from '../../data';
...
export default function MongodbLessonPage({ params }: PageProps) {
  const { slug } = use(params);
  const lesson = mongodbLessons[slug];
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto p-6 md:p-12">
      ...
      {/* Navigation */}
      <LessonNavigation 
        prevLink={lesson.prevLesson ? `/lessons/mongodb/${lesson.prevLesson}` : undefined}
        prevTitle={lesson.prevLesson ? mongodbLessons[lesson.prevLesson]?.title : undefined}
        nextLink={lesson.nextLesson ? `/lessons/mongodb/${lesson.nextLesson}` : undefined}
        nextTitle={lesson.nextLesson ? mongodbLessons[lesson.nextLesson]?.title : undefined}
      />
    </div>
  );
}
