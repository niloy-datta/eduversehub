...
import { dsaJavaLessons, Section, TableContent, QuizQuestion, QuizOption } from '../../data';
...
export default function DsaJavaLessonPage({ params }: PageProps) {
  const { slug } = use(params);
  const lesson = dsaJavaLessons[slug];
  ...
  const renderSection = (section: Section, index: number) => {
    ...
    case 'table':
        if (typeof section.content === 'object' && !Array.isArray(section.content) && 'headers' in section.content) {
          const table = section.content as unknown as TableContent;
          return (
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto p-6 md:p-12">
      ...
      {/* Navigation */}
      <LessonNavigation 
        prevLink={lesson.prevLesson ? `/lessons/dsa-java/${lesson.prevLesson}` : undefined}
        prevTitle={lesson.prevLesson ? dsaJavaLessons[lesson.prevLesson]?.title : undefined}
        nextLink={lesson.nextLesson ? `/lessons/dsa-java/${lesson.nextLesson}` : undefined}
        nextTitle={lesson.nextLesson ? dsaJavaLessons[lesson.nextLesson]?.title : undefined}
      />
    </div>
  );
}
