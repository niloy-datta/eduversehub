...
import { springBootLessons, QuizQuestion, Section, TableContent } from '../../data';
...
export default function SpringBootLessonPage({ params }: PageProps) {
  const { slug } = use(params);
  const lesson = springBootLessons[slug];
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto p-6 md:p-12">
        ...
        <div className="prose prose-invert max-w-none animate-fade-in">
          ...
          {lesson.quiz && (
            <div className="mt-8 p-6 bg-dark-900 rounded-xl border border-dark-800 text-center">
              ...
              <p className="text-dark-400 mb-4">Take a quick quiz to verify what you learned.</p>
              <button 
                onClick={() => setActiveTab('quiz')}
            ...
            <div key={q.id} className="bg-dark-900/50 p-6 rounded-2xl border border-dark-800">
              ...
              <div className="grid gap-3">
                {q.options.map((opt) => {
                  ...
                  const isCorrectResult = q.correctAnswer === opt.id;
                  const showCorrect = showResults && isCorrectResult;
                  const showWrong = showResults && isSelected && !isCorrectResult;

                  let btnClass = "w-full text-left p-4 rounded-xl transition-all border border-dark-700 bg-dark-800 hover:bg-dark-700";
                  
                  if (showResults) {
                    if (isCorrectResult) btnClass = "w-full text-left p-4 rounded-xl border border-success-500 bg-success-500/10 text-success-200";
                    else if (isSelected && !isCorrectResult) btnClass = "w-full text-left p-4 rounded-xl border border-error-500 bg-error-500/10 text-error-200";
      ...
      {/* Navigation */}
      <LessonNavigation 
        prevLink={lesson.prevLesson ? `/lessons/spring-boot/${lesson.prevLesson}` : undefined}
        prevTitle={lesson.prevLesson ? springBootLessons[lesson.prevLesson]?.title : undefined}
        nextLink={lesson.nextLesson ? `/lessons/spring-boot/${lesson.nextLesson}` : undefined}
        nextTitle={lesson.nextLesson ? springBootLessons[lesson.nextLesson]?.title : undefined}
      />
    </div>
  );
}
