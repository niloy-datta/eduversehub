...
import { javascriptLessons, Section } from '../../data';
...
export default function JavaScriptLessonPage({ params }: PageProps) {
  const { slug } = use(params);
  const lesson = javascriptLessons[slug];
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto p-6 md:p-12">
      ...
      {/* Navigation */}
      <LessonNavigation 
        prevLink={lesson.prevLesson ? `/lessons/javascript/${lesson.prevLesson}` : undefined}
        prevTitle={lesson.prevLesson ? javascriptLessons[lesson.prevLesson]?.title : undefined}
        nextLink={lesson.nextLesson ? `/lessons/javascript/${lesson.nextLesson}` : undefined}
        nextTitle={lesson.nextLesson ? javascriptLessons[lesson.nextLesson]?.title : undefined}
      />
    </div>
  );
}
