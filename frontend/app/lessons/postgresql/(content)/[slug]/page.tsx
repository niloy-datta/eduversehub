...
import { postgresqlLessons, Section, TableContent, QuizQuestion, QuizOption } from '../../data';
...
export default function PostgresqlLessonPage({ params }: PageProps) {
  const { slug } = use(params);
  const lesson = postgresqlLessons[slug];
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto p-6 md:p-12">
      ...
      {/* Navigation */}
      <LessonNavigation 
        prevLink={lesson.prevLesson ? `/lessons/postgresql/${lesson.prevLesson}` : undefined}
        prevTitle={lesson.prevLesson ? postgresqlLessons[lesson.prevLesson]?.title : undefined}
        nextLink={lesson.nextLesson ? `/lessons/postgresql/${lesson.nextLesson}` : undefined}
        nextTitle={lesson.nextLesson ? postgresqlLessons[lesson.nextLesson]?.title : undefined}
      />
    </div>
  );
}
