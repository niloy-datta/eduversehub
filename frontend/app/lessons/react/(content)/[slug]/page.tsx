...
export default function ReactLessonPage({ params }: PageProps) {
  const { slug } = use(params);
  const lesson = reactLessons[slug];
            ...
            <div key={index} className="mb-6 overflow-x-auto">
              ...
              <table className="w-full text-left border-collapse border border-dark-700 rounded-lg overflow-hidden">
                ...
                <tbody className="bg-dark-900/50 text-dark-300 text-sm">
                  {(section.content.rows as string[][]).map((row: string[], i: number) => (
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto p-6 md:p-12">
            ...
            <div key={q.id} className="bg-dark-900/50 p-6 rounded-2xl border border-dark-800">
              ...
              <div className="grid gap-3">
                {q.options.map((opt: any) => {
                  ...
                  const isCorrectResult = (lesson.quiz as any)[idx].correctAnswer === opt.id; 
                  // Wait, logic above used q.correctAnswer.
                  // But mapping lesson.quiz... q IS the question object?
                  // Yes.
                  const isCorrect = q.correctAnswer === opt.id;
                  
                  // Re-using logic from spring-boot page visualization:
      ...
      {/* Navigation */}
      <LessonNavigation 
        prevLink={lesson.prevLesson ? `/lessons/react/${lesson.prevLesson}` : undefined}
        prevTitle={lesson.prevLesson ? reactLessons[lesson.prevLesson]?.title : undefined}
        nextLink={lesson.nextLesson ? `/lessons/react/${lesson.nextLesson}` : undefined}
        nextTitle={lesson.nextLesson ? reactLessons[lesson.nextLesson]?.title : undefined}
      />
    </div>
  );
}
