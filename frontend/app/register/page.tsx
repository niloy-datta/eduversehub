    ...
    <div className="min-h-screen bg-dark-950 flex items-center justify-center px-4 py-12">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="glass rounded-3xl p-8">
          ...
          <p className="text-dark-400 text-center mb-8">একটি নতুন অ্যাকাউন্ট তৈরি করুন</p>

          {error && (
            <div className="mb-6 p-4 rounded-xl bg-error-500/20 border border-error-500/50">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-dark-800/50 border border-dark-700 text-white placeholder-dark-500 focus:outline-none focus:border-primary-500 transition-colors pr-12"
                  placeholder="••••••••"
                  required
                  minLength={6}
                />
                ...
                <div className="mt-2">
                  <div className="flex gap-1 mb-1">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <div
                        key={i}
                        className={`flex-1 h-1 rounded-full ${i <= strength.score ? strength.color : 'bg-dark-700'}`}
                      />
                    ))}
                  </div>
                  <span className="text-xs text-dark-400">{strength.label}</span>
                </div>
              )}
            </div>

            <div>
              <label className="block text-dark-300 text-sm font-medium mb-2">Confirm Password</label>
              <input
                type={showPassword ? 'text' : 'password'}
                value={confirmPassword}
                onChange={(e) => setConfirmPassword(e.target.value)}
                className={`w-full px-4 py-3 rounded-xl bg-dark-800/50 border text-white placeholder-dark-500 focus:outline-none transition-colors ${
                  confirmPassword && confirmPassword !== password ? 'border-error-500' : 'border-dark-700 focus:border-primary-500'
                }`}
                placeholder="••••••••"
                required
              />
              {confirmPassword && confirmPassword !== password && (
                <span className="text-xs text-error-400">Passwords do not match</span>
              )}
            </div>
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              <input
                ...
                className="w-4 h-4 mt-1 rounded border-dark-700 bg-dark-800 text-primary-500 focus:ring-primary-500"
              />
          ...
          <div className="grid grid-cols-2 gap-4">
            <button className="flex items-center justify-center gap-2 py-3 rounded-xl bg-dark-800/50 border border-dark-700 text-white hover:bg-dark-700/50 transition-colors">
